...
impl GpuSamplerKernels {
    pub fn new(ctx: &MetalContext) -> Result<Self, Error> {
        Ok(Self {
            ...
            scatter_vocab_chunk: ctx.compile_kernel(SCATTER_VOCAB_CHUNK_SHADER, "scatter_vocab_chunk")?,
            ...
            softmax_rows: ctx.compile_subkernel(softmax_rows::shader_source(), softmax_rows::ENTRY, KernelVariant::PRODUCTION)?,
            sample_from_probs_rows: ctx.compile_kernel(SAMPLE_FROM_PROBS_SHADER, "sample_from_probs_rows")?,
        })
    }
}
