...
/// Occupancy-first 32x32x32 prototype (`gemm_block_sq`, bench-only): q4 + raw
/// bf16 variants at ~9KB threadgroup memory vs gemm_block's 20KB. Research
/// probe for the MPS/MLX GEMM gap.
pub fn bench_gemm_block_sq(shapes: &[GemmShape], iters: usize) -> Result<Vec<GemmBenchRow>, Error> {
    use crate::kernels::sub::gemm_q4;
    use crate::metal::device::MetalContext;
    use objc2_metal::{MTLBuffer, MTLCommandBuffer, MTLCommandEncoder, MTLCommandQueue, MTLComputeCommandEncoder, MTLSize};

    const SHADER_SQ: &str = shader_include::include_metal!("kernels/gemm_block_sq.metal");
    let ctx = MetalContext::new()?;
    let warmup = 3usize;
    let mut rows = Vec::new();
    let mut pool = BufferPool::new();
    ...
    for &shape in shapes {
        ...
        let buf_x = pool
            .allocate(&ctx.device, m * k * 2)
            .ok_or(Error::Format("bench sq x"))?;
        let buf_y = pool
            .allocate(&ctx.device, m * n * 2)
            .ok_or(Error::Format("bench sq y"))?;
        let buf_w = pool
            .allocate(&ctx.device, w_q4.len())
            .ok_or(Error::Format("bench sq w"))?;
        BufferPool::write_bf16(&buf_x, &bf16::f32_slice_to_bf16_bits(&fixture.x));
        BufferPool::write_bytes(&buf_w, &w_q4);
        ...
        rows.push(GemmBenchRow {
            shape,
            label: format!("gemm_block_sq/batched_x{iters}"),
            gflops: rate,
        });

        // f16-activation variant (x_fp16 FC: reinterpret loads, no bf16
        // shift-convert per element) — isolates the A-conversion cost.
        let pipeline_f16 = ctx.compile_gemm_subkernel(
            SHADER_SQ,
            "gemm_block_sq",
            n as u32,
            k as u32,
        ...
        BufferPool::write_bf16(&buf_x, &crate::kernels::sub::f16::f32_slice_to_f16(&fixture.x));
        let dispatch_f16 = |count: usize| -> Result<(), Error> {
            let cmd = ctx.queue.commandBuffer().ok_or(Error::Format("cmd"))?;
            let enc = cmd.computeCommandEncoder().ok_or(Error::Format("enc"))?;
            for _ in 0..count {
                enc.setComputePipelineState(&pipeline_f16.pipeline);
                gemm_q4::bind_gpu_buffers(&enc, &buf_x, &buf_y, &buf_w, 0, m as u32);
                enc.dispatchThreadgroups_threadsPerThreadgroup(grid, tg);
            }
            enc.endEncoding();
        ...
        // Correctness check vs the production kernel (tolerance compare:
        // 64x64 tiling changes accumulation grouping, not the K order, but
        // bf16 output rounding after different tile staging can differ 1ulp).
        {
            let ref_pipe = crate::kernels::sub::gemm_q4::pipeline_for(&ctx, n as u32, k as u32)?;
            let buf_ref = pool
                .allocate(&ctx.device, m * n * 2)
                .ok_or(Error::Format("bench sq ref"))?;
            BufferPool::write_bf16(&buf_x, &bf16::f32_slice_to_bf16_bits(&fixture.x));
            let cmd = ctx.queue.commandBuffer().ok_or(Error::Format("cmd"))?;
            let enc = cmd.computeCommandEncoder().ok_or(Error::Format("enc"))?;
            ...
            for i in 0..(m * n) {
                let a = bf16::bf16_bits_to_f32(unsafe { *ry.add(i) });
                let b = bf16::bf16_bits_to_f32(unsafe { *sy.add(i) });
            ...
            eprintln!("  sq correctness {m}x{k}x{n}: cos={cos:.6} max|d|={maxd:.5}");
        ...
        rows.push(GemmBenchRow {
            shape,
            label: format!("gemm_block_sq_xf16/batched_x{iters}"),
            gflops: rate,
        });
