...
impl GpuKvCache {
    ...
    pub fn new(
        ...
        for layer in 0..cfg.num_hidden_layers {
            ...
            let keys = device
                ...
                .ok_or(Error::Gpu("GPU kv keys alloc failed"))?;
            let values = device
                .newBufferWithLength_options(bytes, MTLResourceOptions::StorageModeShared)
                .ok_or(Error::Gpu("GPU kv values alloc failed"))?;
            zero_buffer(&keys);
            zero_buffer(&values);
    ...
    pub fn sync_all_from_cpu(&mut self, cpu: &KvCache, canvas_len: usize) -> Result<(), Error> {
        if cpu.kv_len + canvas_len > self.layers.first().map(|l| l.capacity_tokens).unwrap_or(0) {
            return Err(Error::Runtime("GPU kv cache capacity exceeded"));
        }
        self.kv_len = cpu.kv_len;
        for layer in 0..cpu.layers.len() {
            let cpu_layer = cpu
                .layer(layer)
                .ok_or(Error::Runtime("missing cpu kv layer"))?;
            let gpu_layer = self
                .layers
                .get_mut(layer)
                .ok_or(Error::Gpu("missing gpu kv layer"))?;
...
fn sync_prefix(gpu: &mut GpuKvLayer, kv: LayerKvView<'_>) -> Result<(), Error> {
    ...
    if kv.n_kv_heads * kv.head_dim != kv_dim {
        return Err(Error::Gpu("gpu kv dim mismatch"));
    }
    ...
    if kv.keys.len() < elems || kv.values.len() < elems {
        return Err(Error::Runtime("cpu kv prefix too short"));
    }
