//! The pure decoding core: a `StreamDecoder` that turns raw per-step canvas
//! snapshots (`StepProgressEvent`) into semantic [`ChatEvent`]s. Decoupling
//! the events from rendering makes the stream observable (emit it to a file
//! with `--events`, or stdout with `--json`) and makes the fragile
//! stable-prefix logic unit-testable.
