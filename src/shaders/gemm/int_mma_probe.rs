...
//! measures per-instruction throughput in an isolated register loop, not the
//! per-instruction WIDTH × accumulation-chain serial dependency × dispatch
//! tile that determines real GEMM throughput. The real prototype on real MoE
...
//! Do not revive it as a GEMM-shape decision tool.
//!
