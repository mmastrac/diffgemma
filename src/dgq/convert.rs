...
pub fn quantize_model(opts: QuantizeOptions) -> Result<QuantizeSummary, Error> {
    ...
    for (i, name) in names.iter().enumerate() {
        ...
        // Every tensor's canonical start is 64-byte aligned, UNCONDITIONALLY:
        // `gemm_rowk`-class kernels reinterpret `blob + w_off` as typed
        // pointers (`device const ushort *`) and index them — a writer draft
        // that skipped this inside shard runs produced a pack whose GPU-read
        // weights were silently wrong from layer 1 onward despite
        // byte-identical tensor content (byte-granular readers like
        // `host_ptr` have no alignment requirement, so only golden caught
        // it). `assert_tensor_offset_alignment` enforces this at load.
