...
pub fn tiny_fixture(_: ElemFormat) -> Fixture {
    Fixture {
        ...
        b: vec![2.0, 0.5, -1.0, 0.25],
    }
}

pub fn cpu(f: &Fixture) -> Vec<f32> {
    let mut out = f.a.clone();
    for (x, y) in out.iter_mut().zip(f.b.iter()) {
        *x *= *y;
    }
    out
}
...
pub fn gpu(f: &Fixture, variant: KernelVariant) -> Result<Vec<f32>, Error> {
    ...
    let len = f.len();
    let buf_a = pool.allocate(&ctx.device, len * 4).ok_or(Error::Format("alloc"))?;
    ...
    gpu_common::dispatch_1d(&ctx.queue, &pipeline.pipeline, len, |enc| {
        bind_gpu_buffers(enc, &buf_a, &buf_b, &buf_d, len as u32);
    })?;
    let mut out = vec![0.0f32; len];
    BufferPool::read_f32(&buf_a, &mut out);
    Ok(out)
}
...
mod tests {
    ...
    kernel_oracle_matrix! {
        mod tiny,
        cpu = crate::kernels::sub::vec_mul_inplace::cpu,
        cpu_oracle = crate::kernels::sub::vec_mul_inplace::cpu_oracle,
        gpu = crate::kernels::sub::vec_mul_inplace::gpu,
        fixture = crate::kernels::sub::vec_mul_inplace::tiny_fixture,
        out_len = crate::kernels::sub::vec_mul_inplace::fixture_len,
        formats: [F32],
        max_tol = 1e-6,
        min_cos = 0.9999,
    }
