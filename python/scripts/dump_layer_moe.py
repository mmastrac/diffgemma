#!/usr/bin/env python3
"""Dump layer FFN/MoE checkpoints from MLX for monolithic step-moe-dump parity."""
...
def dump_mlx_layer_moe(args: argparse.Namespace) -> dict:
    ...
    residual2 = post_attn
    h1 = target.pre_feedforward_layernorm(residual2)
    h1 = target.mlp(h1)
    dense_out = target.post_feedforward_layernorm_1(h1)
    mx.eval(dense_out)
    ...
    return {
        ...
        "layer_out": [float(x) for x in layer_out[0, pos].tolist()],
    }
...
def main() -> int:
    ...
    try:
        payload = dump_mlx_layer_moe(args)
    ...
    print(
        ...
        f"kv_len={payload['kv_len']}, experts={payload['experts']})"
    )
    return 0
