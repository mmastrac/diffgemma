//! Compile every src/**/*.cu to a cubin for the CUDA backend.
//!
...
//! a CUDA dispatch reports that they were not built. DGQ_NVCC / DGQ_CUDA_ARCH
//! override the compiler and the -arch value.
...
fn main() {
    println!("cargo:rustc-check-cfg=cfg(dgemm_cuda_kernels)");
    ...
    let Some(nvcc) = find_nvcc() else {
        println!(
            "cargo:warning=nvcc not found; dgemm builds without CUDA kernels \
    ...
    println!(
        "cargo:warning=dgemm: compiled {} CUDA kernel source(s) for {arch}",
        sources.len()
    );
    println!("cargo:rustc-cfg=dgemm_cuda_kernels");
}
