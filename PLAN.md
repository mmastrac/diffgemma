...
| ~~"Un-RoPE" the KV for TurboQuant (E8 revisit)~~ | **DISPROVEN TWICE 2026-07-10 (E8-M0, `e8_prerope_k_quant_stats` in step_kv.rs, kept as a diagnostic).** (1) Symbolically, the premise was wrong: K's QK-norm applies a LEARNED per-channel scale γ between W_k and RoPE, and the kernel's rms is computed on the raw W_k output — folding H (or H·diag(γ)) into W_k breaks the normalizer (`rms(M·k_raw) ≠ rms(k_raw)`), so the offline fold is blocked by γ independent of RoPE (V folds precisely because its norm is scale-free). (2) Empirically, pre-RoPE K quantizes IDENTICALLY to post-RoPE (post/pre 0.99-1.00x for q8/q4-sym/q4-affine/±Hadamard, both layer classes) — RoPE's 2×2 pair rotations don't move group-32 ranges in aggregate. Real-K numbers re-confirm the synthetic M1 probe: q8 0.006-0.007, q4-affine 0.085-0.091, rotation ~1.09x, q4 ~13x worse than q8. Parked |
