...
- **Early stop pad masking (2026-06, monolithic):** Prefix-only argmax stability and active-position mean entropy (`ids != pad/filler`) gate `confident_stable`. HF `StableAndConfidentStoppingCriteria` may use full-canvas stats — **not verified spec-parity**; we intentionally ignore pad/filler positions for stability/entropy until HF parity is confirmed. Accept-**mask** plateau (not count; threshold 8, `prefix_mean < 0.05`) is a stuck-canvas backstop only. `DGQ_LOG_EARLY_STOP=1` logs `reason=confident_stable|plateau_stop`. `DGQ_LOG_FINAL_ENTROPY=1` dumps per-position entropy at block end (pad-tail / EOS convergence diagnosis).
...
| `gemm_block` + `gemm_block_stacked` double-buffered K-tile | ~245 ms (o_proj+dense) + ~157 ms (qkv) | Same ping-pong technique as grouped; `ty` aliased over `tw_buf` to fit 32 KB TG limit; 2→1 barrier/K-tile | `c1ce463` — 1.78→1.71 s/step (~4%); all tier-1 + fusion parity pass |
| fp16 → bf16 unification (KV cache + logits/probs) | KV cache + logits storage (was fp16 with bf16-round pre-pass) | Replaced `device half` with `device ushort` + `arena_load`/`arena_store` across 12 Metal kernels; Rust readback switched `f16_bits_to_f32`→`bf16_bits_to_f32`; test fixtures + CPU oracles flipped to bf16 rounding; GEMM `threadgroup half` tiles and f32 lm_head path left as-is | `pending` — 1.71→1.70 s/step; all in-scope tier-1 + `fusion_matches_unfused_forward_logits` pass |
