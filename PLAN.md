...
| **E17b: f32-side-KV GEMM variant** | Tighten E17 precision toward parity: read the f32 side ring (like `attention_mma_full_side`, FC30) instead of the f16 main cache — needs an f32 fragment tiler (simdgroup_float8x8 loads). Would shrink the golden flag-on flips (currently 3/8, f16-KV drift) back toward byte-identity. Last precondition for default-on. |
