...
| **E17 GEMM-attention for full-layer prefill** (NEW; the surviving prefill lever) | Prefill's 58-73% is the 6 full-attn layers at 1.25 TF/s. In prefill, S fits per 256-row sub-chunk (256×kv×4B ≈ 62MB @60k) → decompose to S=Q·Kᵀ (tunable GEMM, 2.3-4 TF/s at MPS wall) → causal row-softmax kernel → O=P·V (tunable GEMM). Same FLOPs at 2-3× the rate; S traffic ~0.4s @30k. Projected 30k prefill 128.7→~75s ≈ MLX parity, 100k 929→~400-450s. Needs: S arena plane (head-batched), softmax+mask kernel, KV-as-GEMM-B plumbing; denoise keeps `attention_mma_full`. Multi-day; NOT bit-identical → full gate (smoketest 17/17 ×{7,42,123}, longctx 4/4, census). Details: memory `prefill-kernel-levers` |
