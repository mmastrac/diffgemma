...
| **Kernel-surface reduction DONE (2026-07-11)** | tunable is the sole production GEMM path (nvfp4 ported in); legacy `gemm_block*` retired to `shaders/oracle/`, `gemm_block_sparse` + adaptive-M deleted, all 3 GEMM flags (`DGQ_GEMM_TUNABLE`/`DGQ_MOE_BLOCK_SPARSE`/`DGQ_MOE_TILE_ADAPT`) gone. Merges landed: `gemm_rowk`, `gemm_q8_linear` (kxn), `embed_gather`, `gather_rows` triad, converter family → `convert_scale`. `shaders/kernels/` 66→47, all golden 8/8. See KERNELS.md banner + `kernel-dedup-followups` memory. **2026-07-12: unified kernel tree** — everything relocated to `src/shaders/<group>/<kernel>/` (rust + metal + cpu oracle colocated; flat `shaders::<name>` aliases; `shaders/` + `src/kernels/` deleted), golden 8/8 |
