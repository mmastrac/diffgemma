...
| **E17a: head-chunk the S plane** (IN PROGRESS) | Bound E17's prefill scratch: process Q heads in batches of HC (not all 16 at once) so S/P scratch is `[HC][CANVAS][n_pad(max_seq)]` — 4× smaller at HC=4 (~0.4 GiB vs ~1.6 GiB @100k). Kernel takes a `head_base`; grid.z = HC, data offsets use `head_base + tgid.z`, scratch offsets use local `tgid.z`. Sweep HC ∈ {4,8,16} for the memory/overlap knee (one head already saturates the GPU, so few barriers/batch should keep ~1.78×). Precondition for default-on. |
| **E17b: f32-side-KV GEMM variant** | Tighten E17 precision toward parity: read the f32 side ring (like `attention_mma_full_side`, FC30) instead of the f16 main cache — needs an f32 fragment tiler (simdgroup_float8x8 loads). Would shrink the golden flag-on flips (currently 3/8, f16-KV drift) back toward byte-identity. Other precondition for default-on. |
