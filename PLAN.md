...
- **NONDET-SC-1 (blocks tight gating + reproducibility):** generation is nondeterministic at fixed seed — localized to the **chunked SC soft-embed GEMM**. Matched-canvas `generate-monolithic` "sky blue" at seed 42 varies ~13–16 denoise steps run-to-run; step 1 (no soft-embed) is bit-identical, divergence starts at step 2 (first SC step), ~1e-2 in mean entropy, cascading. The **slow SC path** (`DGQ_SC_GEMM=0`, single-dispatch scalar) is fully deterministic; the chunked f32-accumulate path is not. Adding memory barriers between chunk dispatches (memzero→accumulate, sc_prob_cols→GEMM, cross-chunk `+=`) did **not** fix it — so it's not a simple cross-dispatch hazard race; suspect the fp16 simdgroup-matmul `+=` accumulate in `gemm_{bf16,q8}_rowk_acc_f32` or chunk-order float summation. This noise is why the smoketest convergence thresholds need ~±4-step margin and can't be ratcheted tighter. Dedicated investigation needed (it's also a quality/reproducibility concern, cf. determinism invariant in STRATEGY §6).
