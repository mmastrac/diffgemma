...
| ~~"Un-RoPE" the KV for TurboQuant (E8 revisit)~~ | **DISPROVEN 2026-07-10 (E8-M0, `e8_prerope_k_quant_stats` in step_kv.rs, kept as a diagnostic).** Real-K probe (1750-token fast prefill, all 30 layers, exact CPU inverse-RoPE of stored rows): pre-RoPE K quantizes IDENTICALLY to post-RoPE (post/pre 0.99-1.00x for q8/q4-sym/q4-affine/±Hadamard, both layer classes) — RoPE rotates dim PAIRS and doesn't move group-32 ranges in aggregate. Real-K numbers re-confirm the synthetic M1 probe: q8 0.006-0.007, q4-affine 0.085-0.091, rotation buys only ~1.09x, q4 ~13x worse than q8. With the quantize-better half dead, restoring the offline fold alone (saves an already-cheap runtime WHT) can't justify the storage-contract surgery. Parked |
