...
| E15 | **Layer-KV bisect with the 3% criterion** | the defect enters between layer 0 (KV verified engine-clean, rel 0.0025) and mid-stack, in code shared by scalar+MMA attention and all precision configs, absent in the engine, with onset 2.2k–3.2k. The noise anchor converts "chaos-confounded" into a hard test: fast-vs-engine per-layer per-position KV rel > ~3% = the bug's first expression (below = ignorable). Bisect layers 1→5 at 4.2k, then bisect stages within the guilty layer (attention-out vs FF vs MoE planes). Cleared already: rope (L0 KV match), ring wrap (uncap A/B), KV storage precision (f32 side), arena dtype (fp16), chunk batching, per-sub kv_len/blk ranges, mask formulas (row_lo/t_lo vs engine agree on paper) | ~1 d | first (layer, stage, position-band) with >3% divergence identified | none — with a 100×-margin criterion, something must exceed it |
